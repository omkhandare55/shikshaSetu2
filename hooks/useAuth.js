...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    // Try to load GraamVidya or legacy manual user from localStorage
    try {
      const gvSess = localStorage.getItem('gv_session');
      const saved  = localStorage.getItem('ss_manual_user');
      const data   = gvSess || saved;
      if (data) setManual(JSON.parse(data));
    } catch {}
